...
exports.buildFatturaPaXml = buildFatturaPaXml;
...
function xmlOptional(tag, value) {
    const trimmed = value?.trim();
    return trimmed ? `<${tag}>${xmlEscape(trimmed)}</${tag}>` : '';
}
function buildRiferimentoDocumentoXml(ref) {
  ...
  ${xmlOptional('DataDDT', data)}
    ...
    if (!idDocumento && !data && !cig && !cup && !numItem)
        return '';
    const common = `
...
function partyId(party) {
    ...
    if (piva.length === 11)
        return `<IdFiscaleIVA><IdPaese>IT</IdPaese><IdCodice>${xmlEscape(piva)}</IdCodice></IdFiscaleIVA>`;
    const cf = party.codiceFiscale?.trim() || '';
    if (cf)
        return `<CodiceFiscale>${xmlEscape(cf)}</CodiceFiscale>`;
    return '';
}
function validateInput(input) {
    const errors = [];
    if (!input.cedente.partitaIva?.trim())
        errors.push('P.IVA emittente mancante nelle impostazioni azienda.');
    if (!input.cedente.denominazione.trim())
        errors.push('Denominazione emittente mancante.');
    if (!input.cessionario.denominazione.trim())
        errors.push('Denominazione cliente mancante.');
    if (!input.cessionario.codiceDestinatario.trim())
        errors.push('Codice destinatario o PEC cliente mancante.');
    if (!input.righe.length)
        errors.push('La fattura non contiene righe.');
    if (!input.numeroDocumento.trim())
        errors.push('Numero documento mancante.');
    return errors;
}
function buildFatturaPaXml(input) {
    ...
    const pecDest = codDest.length !== 7 && input.cessionario.pec?.trim()
  ...
  ${natura}
    ...
    const pagamento = input.modalitaPagamento || input.iban
        ? `<DatiPagamento>
...
//# sourceMappingURL=fatturaPaXml.js.map