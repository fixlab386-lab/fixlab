...
function splitVatNumber(raw: string): { countryCode: string; vatNumber: string } {
...
function parseItalianAddress(raw: string): { address?: string; cap?: string; city?: string; province?: string } {
...
async function queryVies(countryCode: string, vatNumber: string): Promise<ViesResponse> {
  ...
  try {
    ...
    if (!res.ok) {
      throw new HttpsError('unavailable', 'Servizio di verifica P.IVA non raggiungibile. Riprova più tardi.')
    }
    return (await res.json()) as ViesResponse
  } catch (err) {
    if (err instanceof HttpsError) throw err
    throw new HttpsError('unavailable', 'Verifica P.IVA non riuscita: servizio non raggiungibile.')
  } finally {
    clearTimeout(timeout)
  }
}

export const lookupVat = onCall({ region: 'europe-west1' }, async (request): Promise<VatLookupResult> => {
  if (!request.auth?.uid) {
    throw new HttpsError('unauthenticated', 'Autenticazione richiesta.')
  }

  const { countryCode, vatNumber } = splitVatNumber((request.data as { vatNumber?: string })?.vatNumber ?? '')
  if (vatNumber.length < 8) {
    throw new HttpsError('invalid-argument', 'Inserisci una Partita IVA valida.')
  }
