...
import { downloadHtmlAsPdf, printHtmlInIframe, PRINT_DOCUMENT_CSS } from '../../../../lib/printDocument'

export type FornitoriAnteprimaMeta = {
  title: string
  filename: string
}
...
export default function FornitoriAnteprimaStampaDialog({ innerHtml, meta, initialCopie = 1, onClose }: Props) {
  ...
  const handlePrint = useCallback(() => {
    for (let i = 0; i < copie; i++) {
      printHtmlInIframe(innerHtml, meta.title, PRINT_DOCUMENT_CSS)
    }
  ...
  const handlePdf = useCallback(async () => {
    setBusy(true)
    try {
      await downloadHtmlAsPdf(innerHtml, meta.filename, PRINT_DOCUMENT_CSS)
    } catch {
