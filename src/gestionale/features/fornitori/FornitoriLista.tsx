...
function FilterPopover({
  ...
  const values = uniqueColumnValues(fornitori, col)
...
export default function FornitoriLista({
  fornitori,
  selectedId,
  selectionMode,
  selectedIds,
  ...
  const filtered = useMemo(() => {
    let list = applyColumnFilters(fornitori, filtriColonna)
    if (sortColumn && criterioRaggruppamento === 'Nessuno') {
      list = sortFornitori(list, sortColumn, sortDirection)
    }
    return list
  }, [fornitori, filtriColonna, sortColumn, sortDirection, criterioRaggruppamento])
            ...
            {rows.length === 0 ? (
              <tr>
                <td colSpan={visibleCols.length + 1} className="clienti-empty">
                  Nessun fornitore. Usa «Nuovo» per aggiungerne uno.
                </td>
              </tr>
            ) : null}
            {useVirtual && topPad > 0 ? (
