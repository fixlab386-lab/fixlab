...
import type { Cliente, ContattoExtra, Indirizzo } from '../types'
...
export function ProprietaCompleteDialog({ cliente, onClose }: { cliente: Cliente; onClose: () => void }) {
  const so = cliente.sedeOperativa
  return (
    <div className="clienti-dialog-overlay" onClick={onClose}>
      <div className="clienti-dialog clienti-dialog--lg" onClick={e => e.stopPropagation()}>
        ...
        <div className="clienti-dialog__body" style={{ fontSize: 11, lineHeight: 1.5 }}>
          ...
          <p><strong>Cod. fiscale:</strong> {cliente.codFiscale || '—'}</p>
          <p><strong>Agente:</strong> {cliente.rapportiCommerciali.agente}</p>
          <p><strong>Pagamento:</strong> {cliente.rapportiCommerciali.pagamento || '—'}</p>
          <p><strong>Sedi extra:</strong> {cliente.sediExtra.length}</p>
          <p><strong>Contatti extra:</strong> {cliente.contattiExtra.length}</p>
        </div>
        <div className="clienti-dialog__footer">
          <button type="button" className="clienti-dialog__btn" onClick={onClose}>
            Chiudi
